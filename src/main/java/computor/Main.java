...
//			System.out.println(management.getMapOfPowers(s));
	...
	public static class ArgumentsManagement {
		...
		public void getRootsOfQuadraticEquation(TreeMap<Integer, Double> map) {
			...
			double b = map.get(1) == null ? 0 : map.get(1);
			Double d = b * b - 4 * c * map.get(2);
			if (d > 0) {
				result[0] = (-b + Math.sqrt(d)) / (2 * map.get(2));
				result[1] = (-b - Math.sqrt(d)) / (2 * map.get(2));
				System.out.println("Discriminant is strictly positive, the two solutions are:");
				System.out.println(result[0]);
				System.out.println(result[1]);
			} else if (d == 0) {
				result[0] = -b / (2 * map.get(2));
		...
		public void solveLinearEquation(TreeMap<Integer, Double> map) {
			double a = map.get(0) == null ? 0 : map.get(0);
